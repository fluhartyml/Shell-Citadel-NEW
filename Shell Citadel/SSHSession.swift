...
actor SSHSession {
    ...
    func replyLines(path rawPath: String, startingAtByte startOffset: Int = 0) async throws -> AsyncThrowingStream<ReplyChunk, Error> {
        ...
        // ⛔ THE RESUME CAP — Michael, 2026-09-09 06:40, after the app went down again on
        // an overnight gap: "i want 93 to have the five minute cap warn before pushing".
        //
        // A true five-minute cap is not possible: out.txt carries no timestamps. So the
        // cap is in LINES, and the number is measured rather than guessed — 50 lines came
        // through in the 18 minutes of 06:30–06:48 that morning, so five minutes of live
        // conversation is roughly 15 lines. He chose 100, which is generous and still far
        // below a flood.
        //
        // What it does on a resume: count what is pending, and if it exceeds the cap,
        // move the start to the last \(Self.resumeLineCap) lines and SAY SO before any of
        // it arrives. The skipped text is not lost — it is still in the file on the Mac.
        //
        // ⚠️ THE SENTINEL IS PRINTED AFTER THE ADJUSTMENT, NOT BEFORE. The app seeds its
        // consumed-count from that number, so it must be the offset actually followed or
        // the next resume replays everything this one skipped.
        let raw = try await client.executeCommandStream(
            "mkdir -p \"$(dirname \(path))\" && touch \(path) && "
                + "__sc_start=\(startExpr); "
                + "__sc_pending=$(tail -c +$((__sc_start + 1)) \(path) | wc -l | tr -d ' '); "
                + "__sc_skipped=0; "
                + "if [ \"$__sc_pending\" -gt \(Self.resumeLineCap) ]; then "
                + "__sc_skipped=$((__sc_pending - \(Self.resumeLineCap))); "
                + "__sc_tailbytes=$(tail -c +$((__sc_start + 1)) \(path) | tail -n \(Self.resumeLineCap) | wc -c | tr -d ' '); "
                + "__sc_size=$(wc -c < \(path) | tr -d ' '); "
                + "__sc_start=$((__sc_size - __sc_tailbytes)); "
                + "fi; "
                + "printf '\(Self.offsetSentinel)%s\\n' \"$__sc_start\"; "
                + "if [ \"$__sc_skipped\" -gt 0 ]; then "
                + "printf '\(Self.skippedNotice)%s\\n' \"$__sc_skipped\"; "
                + "fi; "
                ...
                do {
                    for try await chunk in raw {
                        ...
                        while let newline = pending.firstIndex(of: "\n") {
                            ...
                            // The skip notice is generated by the far end, not read
                            // from the file, so it costs no bytes and must not be
                            // counted. It is shown as a plain sentence — the whole point
                            // is that he is TOLD instead of flooded.
                            if trimmed.hasPrefix(Self.skippedNotice) {
                                let digits = trimmed.dropFirst(Self.skippedNotice.count)
                                if let n = Int(digits), n > 0 {
                                    continuation.yield(ReplyChunk(
                                        text: "\u{26A0}\u{FE0F} \(n) earlier lines were held back. They are still on the Mac.",
                                        offsetAfter: consumed))
                                }
                                continue
                            }
    ...
    /// Printed instead of a flood, with the number of lines held back on the Mac.
    static let skippedNotice = "__SC_SKIPPED__"

    /// The resume cap, in lines. His number, 2026-09-09 — see the comment at the follow
    /// command for why it is lines and not minutes, and where 100 came from.
    static let resumeLineCap = 100
